...
export default function MapComponents({
  ...
  // Define danger level color mapping
  const dangerColorOpacity = {
    'extreme': { color: '#ef4444', opacity: 0.5 },  // red-500 with higher opacity
    'high': { color: '#f97316', opacity: 0.45 },    // orange-500
    'medium': { color: '#eab308', opacity: 0.4 },   // yellow-500
    'low': { color: '#22c55e', opacity: 0.35 }      // green-500
  };
  ...
  return (
    <MapContainer
      ...
      zoomControl={false}
      className="z-0"
      attributionControl={false} // We'll add this custom in MapController
    >
      ...
      {dangerZones.map((zone) => {
        const dangerStyle = dangerColorOpacity[zone.dangerLevel as keyof typeof dangerColorOpacity] || 
              ...
              <Popup className="custom-popup">
                <div className="w-64 p-1">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2 text-sm bg-gray-50 p-2 rounded">
                    ...
                    {zone.airQuality && <div className="font-medium">Air Quality: {zone.airQuality}</div>}
                  </div>
                  <p className="text-xs text-gray-500 mt-2">
                    {new Date(zone.timestamp).toLocaleString()}
                  </p>
                  <button
                    className="mt-3 w-full py-2 px-3 bg-blue-600 text-white rounded text-sm hover:bg-blue-700 transition shadow-sm"
      ...
      {/* Line to nearest danger zone */}
      {userLocation && nearestDangerZone && nearestDangerZone.distance >= 5 && (
        <Polyline
          positions={[
            userLocation,
            [
              nearestDangerZone.zone.location.lat,
              nearestDangerZone.zone.location.lng,
            ],
          ]}
